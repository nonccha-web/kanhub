...
- **`render()` เด้งขึ้นหัวหน้าเฉพาะตอนเปลี่ยนหน้าจริงๆ** (เทียบ `S.lastHash` กับ `location.hash`)
  วาดหน้าเดิมซ้ำ — ลบงาน กดเสร็จ ติ๊กโพสต์ รับลีด — ต้องอยู่ตรงที่เดิม ไม่งั้นลบทีนึงต้องเลื่อนหาที่เดิมใหม่ทุกครั้ง
  `keepScroll()` เฝ้า `#view` แล้วดันกลับที่เดิมหลังเนื้อหาถูกเขียนทับ (เนื้อหามาทีหลังเพราะต้องรอ API)
  และ**เลิกยุ่งทันทีที่คนขยับจอเอง** (wheel/touchmove/keydown) — ห้ามแย่งจอกับคน
...
## ปฏิทินการตลาด — สามมุมมอง (ปฏิทิน / ตาราง / สไลด์)

สวิตช์อยู่ **แถบบนสุด** คู่กับปุ่มสำรองไฟล์/เพิ่มรายการ (`#ccModeSw` ใน html — ไม่ใช่ชิปกรองด้านล่าง
เคยวางไว้ตรงนั้นแล้วนนท์หาไม่เจอ เพราะหน้าตาเหมือนตัวกรอง ไม่เหมือนสวิตช์มุมมอง)

**ปฏิทิน** = กริดเดือนแบบเดิม เห็นว่าอะไรทับกันช่วงไหน
**ตาราง** = แบบ Lark Base — จัดกลุ่มได้ พับได้ พิมพ์เพิ่มในกลุ่มได้เลย
**สไลด์** = `campaign-deck.html` คนละหน้า (มีลิงก์กลับ) — สลับไปมาได้จากสวิตช์เดียวกัน

จำมุมมองล่าสุดไว้ใน `localStorage` (`kan-cc-layout` / `kan-cc-group`) · ตัวกรองประเภทกับสาขาใช้ร่วมกันทั้งสองมุมมอง
แถวใต้สถิติเหลือแค่ "จัดกลุ่มตาม" และขึ้นเฉพาะตอนอยู่มุมมองตาราง
...
### ทำสำเนา (duplicate)
โปรฯ ส่วนใหญ่ก๊อปจากของเก่ามาแก้วัน — ปุ่ม **ทำสำเนา** อยู่ 3 ที่: ทุกแถวในตาราง · การ์ดที่เด้งตอนกดแถบในปฏิทิน · ในฟอร์มตอนแก้ไข

กดแล้ว **เปิดฟอร์มของ "รายการใหม่"** โดยยกค่าจากใบเก่ามาให้ครบ ยังไม่บันทึก —
ตั้งใจไม่บันทึกทันที เพราะก๊อปโปรฯ เก่ามาทั้งดุ้นโดยไม่เปลี่ยนวันแทบไม่มีประโยชน์

- ชื่อเติม `(สำเนา)` · ก๊อปสำเนาอีกทีได้ `(สำเนา 2)` ไม่ต่อท้ายซ้อนกันรัว (`dupName()`)
- **สถานะรีเซ็ตเป็น "วางแผน" เสมอ** — ก๊อปโปรฯ ที่จบแล้วมาแล้วขึ้นว่าจบแล้วตั้งแต่ยังไม่ทำ = อ่านผิด
- **รูปตามมาด้วย** (`copyAttachments()`) — ดึงไฟล์เดิมมาเป็นรูป "ยังไม่บันทึก" ของใบใหม่ อัปขึ้นจริงตอนกดบันทึก
  จึงได้ไฟล์คนละใบกับต้นฉบับ ลบรูปในสำเนาไม่กระทบของเดิม
- `renderAttachments()` / `renderLinks()` ต้องส่ง `null` ตอนทำสำเนา — **ถ้าส่งใบต้นฉบับไป
  ปุ่มลบรูปจะชี้ไปที่ไฟล์ของใบเก่า กดแล้วไปลบรูปต้นฉบับทิ้ง**
- โพสต์/งานที่ผูกกับใบเก่าไม่ถูกก๊อป (ผูกกับ id ของใบนั้น) — สำเนาเริ่มจากไม่มีสื่อ
